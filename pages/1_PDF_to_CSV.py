...
from csv_format import STANDARD_COLUMNS, normalized_to_standard
from pdf_parser import extract_pdf_debug_text
from pdf_to_csv import extract_pdfs, pdfs_to_csv_bytes
...
st.caption(
    ...
    "Step 2: Use that CSV in **Spending Analyzer**."
)
...
if not uploaded_pdfs:
    ...
    with st.expander("How this works"):
        st.markdown(
            ...
            4. Open **Spending Analyzer** in the sidebar and upload the CSV
    ...
    with st.expander("Privacy"):
        st.markdown(
            ...
            - Download the CSV and use it in Spending Analyzer
            - Optional online merchant lookup (in Spending Analyzer) is separate and off by default
            """
        )
    st.stop()
...
st.download_button(
    label="Download CSV",
    data=pdfs_to_csv_bytes(uploaded_pdfs),
    file_name=csv_name,
    mime="text/csv",
    type="primary",
)
...
st.caption("When this looks right, download the CSV and upload it in **Spending Analyzer**.")
